...
class DownloaderRepositoryImpl implements IDownloaderRepository {
  ...
  Future<void> _scanLibrary() async {
    try {
      ...
      final extractor = MetadataExtractorService(BinaryLocator());
      final files = dir.listSync();
      for (final entity in files) {
        if (entity is File) {
          final filename = entity.uri.pathSegments.last;
          if (_isVideo(filename)) {
            final known = _activeDownloads.values.any(
              (d) => d.filePath == entity.path || d.title == filename,
            );
            if (!known) {
              ...
              String requestUrl = 'imported';
              if (metadata?.sourceUrl != null) {
                requestUrl = metadata!.sourceUrl!;
              }
              ...
              final newItem = DownloadItem(
                ...
                sortOrder: 9999,
                downloadedSize: _formatBytes(entity.lengthSync()),
                totalSize: _formatBytes(entity.lengthSync()),
              );
              _activeDownloads[id] = newItem;
