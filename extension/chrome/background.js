...
async function detectBrowser() {
    ...
    const res = await api.storage.local.get(['preferredBrowser']);
    if (res.preferredBrowser && res.preferredBrowser !== 'auto') {
        return res.preferredBrowser;
    }

    // Basic heuristic based on UA and features
    ...
    // Brave (Check for navigator.brave) - Note: this might not work in Service Worker context easily
    if (navigator.brave && await navigator.brave.isBrave()) return 'brave';

    // Default to chrome if unsure (Chromium based)
    return 'chrome';
}
